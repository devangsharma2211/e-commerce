...
<body>
    ...
    <div class="container1" id="rowitems">
        <div class="form-box">
            <h1 id="title">Seller Registration</h1>
            <form action="" method="post">
                <div class="input-group">
                    ...
                    <i class="fa-solid fa-id-badge"></i>
                        <input type="text" placeholder="Enrollment Number" name="enroll" id="enroll" autocomplete="off" required="required" >
                    </div>
...
if (isset($_POST['signup'])) {
    ...
    $ip=getIPAddress();
    //select query
    $select_query = "SELECT * FROM `seller_registration` WHERE enroll='$enroll'";
    $result=mysqli_query($con,$select_query);
    $rows_count=mysqli_num_rows($result);
    if($rows_count>0){
        echo "<script>alert('Seller already exist. Please SignIn')</script>";
            echo "<script>window.open('index.php','_self')</script>";

    }
    else{

        $insert_query="INSERT INTO `seller_registration`( `username`, `email`, `enroll`, `password`, `status`, `ip`) VALUES ('$username','$email','$enroll','$hash_password','$status','$ip')";
